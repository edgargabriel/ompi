...
#include "fcoll_two_phase_gen2.h"
...
#define DEBUG_ON 0
#define FCOLL_TWO_PHASE_GEN2_SHUFFLE_TAG   123
...
int mca_fcoll_two_phase_gen2_break_file_view ( struct iovec *decoded_iov, int iov_count, 
...
int mca_fcoll_two_phase_gen2_get_configuration (mca_io_ompio_file_t *fh, int *two_phase_gen2_num_io_procs, 
                                              int **ret_aggregators);
...
int mca_fcoll_two_phase_gen2_split_iov_array ( mca_io_ompio_file_t *fh, mca_io_ompio_io_array_t *work_array,
                                             int num_entries, int *last_array_pos, int *last_pos_in_field,
                                             int chunk_size );


static int mca_fcoll_two_phase_gen2_minmax ( mca_io_ompio_file_t *fh, struct iovec *iov, int iov_count,  int num_aggregators, long *new_stripe_size);


int mca_fcoll_two_phase_gen2_file_write_all (mca_io_ompio_file_t *fh,
                                      const void *buf,
                                      int count,
                                      ...
                                      ompi_status_public_t *status)
{
    ...
    int *displs = NULL;
    int two_phase_gen2_num_io_procs;
    size_t max_data = 0;
     ...
     **************************************************************************/
    fh->f_get_num_aggregators ( &two_phase_gen2_num_io_procs );
    ...
    ret = mca_fcoll_two_phase_gen2_get_configuration (fh, &two_phase_gen2_num_io_procs, &aggregators);
    if (OMPI_SUCCESS != ret){
	goto exit;
    }
    
    aggr_data = (mca_io_ompio_aggregator_data **) malloc ( two_phase_gen2_num_io_procs * 
                                                           sizeof(mca_io_ompio_aggregator_data*));
    
    for ( i=0; i< two_phase_gen2_num_io_procs; i++ ) {
    ...
    ret = mca_fcoll_two_phase_gen2_minmax ( fh, local_iov_array, local_count,  two_phase_gen2_num_io_procs, &new_stripe_size);
    ...
    total_bytes_per_process = (MPI_Aint*)malloc
        (two_phase_gen2_num_io_procs * fh->f_procs_per_group*sizeof(MPI_Aint));
...
#endif
    if ( 1 == mca_fcoll_two_phase_gen2_num_groups ) {
        ret = fh->f_comm->c_coll.coll_allgather (broken_total_lengths,
						  two_phase_gen2_num_io_procs,
						  MPI_LONG,
    ...
    for ( i=0; i<two_phase_gen2_num_io_procs; i++ ) {
        ...
        for (j=0 ; j<fh->f_procs_per_group ; j++) {
            broken_total_lengths[i] += total_bytes_per_process[j*two_phase_gen2_num_io_procs + i];
        }
#if DEBUG_ON
    ...
    result_counts = (int *) malloc ( two_phase_gen2_num_io_procs * fh->f_procs_per_group * sizeof(int) );
    ...
    for ( i=0; i< two_phase_gen2_num_io_procs; i++ ) {
        ...
        for ( j=0; j <fh->f_procs_per_group; j++ ) {
            aggr_data[i]->fview_count[j] = result_counts[two_phase_gen2_num_io_procs*j+i];
        }
    ...
    reqs1 = (ompi_request_t **)malloc ((fh->f_procs_per_group + 1 )*two_phase_gen2_num_io_procs *sizeof(ompi_request_t *));
    reqs2 = (ompi_request_t **)malloc ((fh->f_procs_per_group + 1 )*two_phase_gen2_num_io_procs *sizeof(ompi_request_t *));
    if ( NULL == reqs1 || NULL == reqs2 ) {
        opal_output (1, "OUT OF MEMORY\n");
        ret = OMPI_ERR_OUT_OF_RESOURCE;
        goto exit;
    }
    for (l=0,i=0; i < two_phase_gen2_num_io_procs; i++ ) {
    ...
    for (index = 1; index < cycles; index++) {
        ...
        SWAP_AGGR_POINTERS(aggr_data,two_phase_gen2_num_io_procs); 
        ...
        ret = ompi_request_wait_all ( (fh->f_procs_per_group + 1 )*two_phase_gen2_num_io_procs, 
    ...
    nentry.nprocs_for_coll = two_phase_gen2_num_io_procs;
...
static int write_init (mca_io_ompio_file_t *fh, int aggregator, mca_io_ompio_aggregator_data *aggr_data, int write_chunksize )
{
    ...
    if ( aggregator == fh->f_rank && aggr_data->prev_num_io_entries) {
        while ( aggr_data->prev_bytes_to_write > 0 ) {                    
            aggr_data->prev_bytes_to_write -= mca_fcoll_two_phase_gen2_split_iov_array (fh, aggr_data->prev_io_array, 
                                                                                      aggr_data->prev_num_io_entries, 
                                                                                      &last_array_pos, &last_pos,
                                                                                      write_chunksize );
            if ( 0 >  fh->f_fbtl->fbtl_pwritev (fh)) {
                free ( aggr_data->prev_io_array);
                opal_output (1, "two_phase_gen2_write_all: fbtl_pwritev failed\n");
                ret = OMPI_ERROR;
                goto exit;
            }
        }
                ...
                if ( 0 < data->disp_index[i] ) {
                    ...
                    if (datatype_size){
                        ret = MCA_PML_CALL(irecv(data->global_buf,
                                                 ...
                                                 FCOLL_TWO_PHASE_GEN2_SHUFFLE_TAG+index,
                                                 data->comm,
                                                 &reqs[i]));
                        if (OMPI_SUCCESS != ret){
                            goto exit;
...
int mca_fcoll_two_phase_gen2_break_file_view ( struct iovec *mem_iov, int mem_count, 
...
int mca_fcoll_two_phase_gen2_get_configuration (mca_io_ompio_file_t *fh, int *two_phase_gen2_num_io_procs, int **ret_aggregators)
{
    int *aggregators=NULL;
    int num_io_procs = *two_phase_gen2_num_io_procs;
    int i;
    ...
    *two_phase_gen2_num_io_procs = num_io_procs;
...
int mca_fcoll_two_phase_gen2_split_iov_array ( mca_io_ompio_file_t *fh, mca_io_ompio_io_array_t *io_array, int num_entries,
