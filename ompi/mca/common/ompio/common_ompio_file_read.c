			      ...
			      ompi_status_public_t *status)
{
    ...
    newpos  = (long)offset + real_bytes_read;
    offset = (OMPI_MPI_OFFSET_TYPE) newpos / fh->f_etype_size;
    mca_io_ompio_file_get_byte_offset (fh->f_fh, offset, &disp);
    newpos = (long) disp;

    io_ompio_log (fh, IO_OMPIO_LOG_EVENT_UPDATE, prevpos, newpos, IO_OMPIO_LOG_READ);
#endif

    return ret;
}

int mca_common_ompio_file_read2 (mca_io_ompio_file_t *fh,
                                 void *buf,
                                 int count,
                                 struct ompi_datatype_t *datatype,
                                 ompi_status_public_t *status)
				 ...
				 ompi_status_public_t * status)
{
    ...
    ret = mca_common_ompio_file_read2 (fh,
				    buf,
				    count,
				    datatype,
				    status);
