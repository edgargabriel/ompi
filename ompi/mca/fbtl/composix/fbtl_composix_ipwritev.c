...
ssize_t  mca_fbtl_composix_ipwritev (ompio_file_t *fh,
				 ompi_request_t *request)
{
#if defined(FBTL_POSIX_HAVE_AIO)
    mca_fbtl_composix_request_data_t *data;
    mca_ompio_request_t *req = (mca_ompio_request_t *) request;
    int i=0, ret;
    off_t start_offset, end_offset, total_length;

    data = (mca_fbtl_composix_request_data_t *) malloc ( sizeof (mca_fbtl_composix_request_data_t));
    if ( NULL == data ) {
    ...
    data->aio_req_chunks = fbtl_composix_max_aio_active_reqs;
    ...
    ret = mca_fbtl_composix_lock( &data->aio_lock, data->aio_fh, F_WRLCK, start_offset, total_length, OMPIO_LOCK_ENTIRE_REGION );
    if ( 0 < ret ) {
        opal_output(1, "mca_fbtl_composix_ipwritev: error in mca_fbtl_composix_lock() error ret=%d %s", ret, strerror(errno));
    ...
    for (i=0; i < data->aio_last_active_req; i++) {
        if (-1 == aio_write(&data->aio_reqs[i])) {
            opal_output(1, "mca_fbtl_composix_ipwritev: error in aio_write():  %s", strerror(errno));
            mca_fbtl_composix_unlock ( &data->aio_lock, data->aio_fh );                    
            free(data->aio_req_status);
            free(data->aio_reqs);
            free(data);
    ...
    req->req_data = data;
    req->req_progress_fn = mca_fbtl_composix_progress;
    req->req_free_fn     = mca_fbtl_composix_request_free;
#endif
    return OMPI_SUCCESS;
}
