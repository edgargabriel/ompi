...
# ------------------------------------------------
AC_DEFUN([MCA_ompi_fbtl_composix_CONFIG],[
    AC_CONFIG_FILES([ompi/mca/fbtl/composix/Makefile])

    fbtl_composix_happy=no
    AC_CHECK_HEADER([aio.h],
                    [dnl NetBSD has aio_* in -lrt, vs. the usual libc
                     OPAL_SEARCH_LIBS_COMPONENT([fbtl_composix],
                                    [aio_write], [rt],
                                    [fbtl_composix_happy="yes"])])

    AC_CHECK_FUNCS([pwritev],[],[])
    AC_CHECK_FUNCS([preadv],[],[])

    AS_IF([test "$fbtl_composix_happy" = "yes"],
          [$1],
          [$2])
])dnl
