...
ssize_t mca_fbtl_composix_ipreadv (ompio_file_t *fh,
			       ompi_request_t *request)
{
    ...
    ret = mca_fbtl_composix_lock( &data->aio_lock, data->aio_fh, F_RDLCK, start_offset, total_length, OMPIO_LOCK_ENTIRE_REGION );
    if ( 0 < ret ) {
        opal_output(1, "mca_fbtl_composix_ipreadv: error in mca_fbtl_composix_lock() error ret=%d  %s", ret, strerror(errno));
    ...
    for (i=0; i < data->aio_last_active_req; i++) {
        if (-1 == aio_read(&data->aio_reqs[i])) {
            opal_output(1, "mca_fbtl_composix_ipreadv: error in aio_read(): %s", strerror(errno));
            mca_fbtl_composix_unlock ( &data->aio_lock, data->aio_fh );            
            free(data->aio_reqs);
            free(data->aio_req_status);
            free(data);
