 ...
 * @file
 */
#ifndef MCA_BTL_SMROCM_H
#define MCA_BTL_SMROCM_H
...
typedef struct mca_btl_smrocm_mem_node_t {
    mca_mpool_base_module_t *sm_mpool; /**< shared memory pool */
} mca_btl_smrocm_mem_node_t;

/**
 * Shared Memory (SM) BTL module.
 */
struct mca_btl_smrocm_component_t {
...
#endif
    struct mca_btl_smrocm_t **sm_btls;
    struct mca_btl_smrocm_frag_t **table;
    size_t sm_num_btls;
    size_t sm_max_btls;
    ...
    int rocm_ipc_verbose;
    int rocm_ipc_output;
    int use_rocm_ipc;
    int use_rocm_ipc_same_gpu;

    unsigned long mpool_min_size;
    char *allocator;
};
typedef struct mca_btl_smrocm_component_t mca_btl_smrocm_component_t;
OPAL_DECLSPEC extern mca_btl_smrocm_component_t mca_btl_smrocm_component;

/**
 * SM BTL Interface
 */
struct mca_btl_smrocm_t {
    mca_btl_base_module_t super; /**< base BTL interface */
    bool btl_inited;             /**< flag indicating if btl has been inited */
    mca_btl_base_module_error_cb_fn_t error_cb;
    mca_rcache_base_module_t *rcache;
};
typedef struct mca_btl_smrocm_t mca_btl_smrocm_t;
OPAL_DECLSPEC extern mca_btl_smrocm_t mca_btl_smrocm;

struct btl_smrocm_pending_send_item_t {
    opal_free_list_item_t super;
    void *data;
};
typedef struct btl_smrocm_pending_send_item_t btl_smrocm_pending_send_item_t;
...
#define VIRTUAL2RELATIVE(VADDR) \
    ((long) (VADDR)             \
     - (long) mca_btl_smrocm_component.shm_bases[mca_btl_smrocm_component.my_smp_rank])
 ...
 */
extern int mca_btl_smrocm_component_progress(void);
...
extern int mca_btl_smrocm_finalize(struct mca_btl_base_module_t *btl);
...
extern int mca_btl_smrocm_add_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
 ...
 *
 */
extern int mca_btl_smrocm_del_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                                    struct opal_proc_t **procs,
                                    struct mca_btl_base_endpoint_t **peers);
 ...
 */
extern int mca_btl_smrocm_free(struct mca_btl_base_module_t *btl,
...
mca_btl_smrocm_prepare_src(struct mca_btl_base_module_t *btl,
                           struct mca_btl_base_endpoint_t *endpoint,
 ...
 * @param peer (IN)     BTL peer addressing
 */
extern int mca_btl_smrocm_sendi(struct mca_btl_base_module_t *btl,
                                struct mca_btl_base_endpoint_t *endpoint,
 ...
 * @param peer (IN)     BTL peer addressing
 */
extern int mca_btl_smrocm_send(struct mca_btl_base_module_t *btl,
                               struct mca_btl_base_endpoint_t *endpoint,
...
/* ROCM IPC control message */
typedef struct ctrlhdr_st {
    enum ipcCtrlMsg ctag;
    int cudev;
} ctrlhdr_t;

/* State of setting up ROCM IPC on an endpoint */
enum ipcState { IPC_INIT = 1, IPC_SENT, IPC_ACKING, IPC_ACKED, IPC_OK, IPC_BAD };

extern void mca_btl_smrocm_dump(struct mca_btl_base_module_t *btl,
                                struct mca_btl_base_endpoint_t *endpoint, int verbose);

#if OPAL_ENABLE_PROGRESS_THREADS == 1
void mca_btl_smrocm_component_event_thread(opal_object_t *);
#endif

#if OPAL_ENABLE_PROGRESS_THREADS == 1
#    define MCA_BTL_SMROCM_SIGNAL_PEER(peer)                                                 \
        {                                                                                    \
            unsigned char cmd = DATA;                                                        \
            if (write(peer->fifo_fd, &cmd, sizeof(cmd)) != sizeof(cmd)) {                    \
                opal_output(0, "mca_btl_smrocm_send: write fifo failed: errno=%d\n", errno); \
            }                                                                                \
        }
#else
#    define MCA_BTL_SMROCM_SIGNAL_PEER(peer)
