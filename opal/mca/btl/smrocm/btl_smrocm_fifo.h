 ...
 * $HEADER$
 */
#ifndef MCA_BTL_SMROCM_FIFO_H
#define MCA_BTL_SMROCM_FIFO_H
...
static void add_pending(struct mca_btl_base_endpoint_t *ep, void *data, bool resend)
{
    ...
    i = opal_free_list_get(&mca_btl_smrocm_component.pending_send_fl);

    /* don't handle error for now */
    assert(i != NULL);

    si = (btl_smrocm_pending_send_item_t *) i;
    si->data = data;
 ...
 */
#define FIFO_MAP(x) ((x) & (mca_btl_smrocm_component.nfifos - 1))
#define FIFO_MAP_NUM(n) \
    ((mca_btl_smrocm_component.nfifos) < (n) ? (mca_btl_smrocm_component.nfifos) : (n))

#define MCA_BTL_SMROCM_FIFO_WRITE(endpoint_peer, my_smp_rank, peer_smp_rank, hdr, resend,         \
                                  retry_pending_sends, rc)                                        \
    do {                                                                                          \
        sm_fifo_t *_fifo = &(mca_btl_smrocm_component.fifo[peer_smp_rank][FIFO_MAP(my_smp_rank)]); \
                                                                                                  \
        if (retry_pending_sends) {                                                                \
            if (0 < opal_list_get_size(&endpoint_peer->pending_sends)) {                          \
                btl_smrocm_process_pending_sends(endpoint_peer);                                  \
            }                                                                                     \
        }                                                                                         \
                                                                                                  \
        ...
        if (sm_fifo_write(hdr, _fifo) != OPAL_SUCCESS) {                                          \
            ...
            MCA_BTL_SMROCM_SIGNAL_PEER(endpoint_peer);                                            \
            rc = OPAL_SUCCESS;                                                                    \
        }                                                                                         \
