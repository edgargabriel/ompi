...
static opal_mutex_t btl_smrocm_accelerator_ipc_lock;
...
static bool smrocm_accelerator_initialized = false;

static void mca_btl_smrocm_accelerator_fini(void);

int mca_btl_smrocm_accelerator_init(void)
{
    int rc = OPAL_SUCCESS;
    int i;
    OBJ_CONSTRUCT(&btl_smrocm_accelerator_ipc_lock, opal_mutex_t);
    ...
    if (OPAL_SUCCESS != rc) {
        opal_output_verbose(1, mca_btl_smrocm_component.rocm_ipc_output, "Failed to create accelerator ipc_stream stream.");
    ...
    if (NULL == accelerator_event_ipc_array) {
        opal_output_verbose(1, mca_btl_smrocm_component.rocm_ipc_output, "No memory.");
        rc = OPAL_ERROR;
        goto cleanup_and_error;
    }
    ...
    for (i = 0; i < accelerator_event_max; i++) {
        ...
        if (OPAL_SUCCESS != rc) {
            opal_output_verbose(1, mca_btl_smrocm_component.rocm_ipc_output, "Accelerator create event failed.");
            rc = OPAL_ERROR;
            goto cleanup_and_error;
        }
    }
    ...
    opal_finalize_register_cleanup(mca_btl_smrocm_accelerator_fini);
    smrocm_accelerator_initialized = true;
...
cleanup_and_error:
    if (OPAL_SUCCESS != rc) {
        ...
        OBJ_DESTRUCT(&btl_smrocm_accelerator_ipc_lock);
    }
...
static void mca_btl_smrocm_accelerator_fini(void)
{
    ...
    if (0 == strcmp(opal_accelerator_base_selected_component.base_version.mca_component_name, "null") ||
        false == smrocm_accelerator_initialized) {
        return;
    }
    ...
    OBJ_DESTRUCT(&btl_smrocm_accelerator_ipc_lock);
    smrocm_accelerator_initialized = false;
}
 ...
 */
int mca_btl_smrocm_progress_one_ipc_event(struct mca_btl_base_descriptor_t **frag)
{   
    int result;
    ...
    OPAL_THREAD_LOCK(&btl_smrocm_accelerator_ipc_lock);
    if (accelerator_event_ipc_num_used > 0) {
        opal_output_verbose(20, mca_btl_smrocm_component.rocm_ipc_output,
                            "smrocm: progress_one_accelerator_ipc_event, outstanding_events=%d",
        ...
        if (OPAL_ERR_RESOURCE_BUSY == result) {
            opal_output_verbose(20, mca_btl_smrocm_component.rocm_ipc_output,
                                "smrocm: event query returned not ready");
            *frag = NULL;
            OPAL_THREAD_UNLOCK(&btl_smrocm_accelerator_ipc_lock);
            return 0;
        } else if (OPAL_SUCCESS != result) {
            opal_output_verbose(1, mca_btl_smrocm_component.rocm_ipc_output,
                            "smrocm: event query failed: %d", result);
            *frag = NULL;
            OPAL_THREAD_UNLOCK(&btl_smrocm_accelerator_ipc_lock);
            return OPAL_ERROR;
        }
        ...
        *frag = accelerator_event_ipc_frag_array[accelerator_event_ipc_first_used];
        opal_output_verbose(10, mca_btl_smrocm_component.rocm_ipc_output, "smrocm: event query returned %d", result);
 ...
 */
int mca_btl_smrocm_memcpy(void *dst, void *src, size_t amount, char *msg,
                           struct mca_btl_base_descriptor_t *frag)
{
    ...
    if (accelerator_event_ipc_num_used == accelerator_event_max) {
        opal_output_verbose(1, mca_btl_smrocm_component.rocm_ipc_output, "smrocm: Out of event handles");
        OPAL_THREAD_UNLOCK(&btl_smrocm_accelerator_ipc_lock);
    ...
    if (accelerator_event_ipc_num_used > accelerator_event_ipc_most) {
        ...
        if (0 == (accelerator_event_ipc_most % 10)) {
            opal_output_verbose(20, mca_btl_smrocm_component.rocm_ipc_output, "smrocm: Maximum ipc events used is now %d",
    ...
    if (OPAL_UNLIKELY(OPAL_SUCCESS != result)) {
        opal_output_verbose(1, mca_btl_smrocm_component.rocm_ipc_output, "smrocm: memcpy async failed: %d",
                            result);
        OPAL_THREAD_UNLOCK(&btl_smrocm_accelerator_ipc_lock);
        return OPAL_ERROR;
    } else {
        opal_output_verbose(20, mca_btl_smrocm_component.rocm_ipc_output,
                            "smrocm: cuMemcpyAsync passed: dst=%p, src=%p, size=%d", dst, src,
                            (int) amount);
    }
    ...
    if (OPAL_UNLIKELY(OPAL_SUCCESS != result)) {
        opal_output_verbose(1, mca_btl_smrocm_component.rocm_ipc_output, "Event Record failed.");
        OPAL_THREAD_UNLOCK(&btl_smrocm_accelerator_ipc_lock);
        return OPAL_ERROR;
    }
