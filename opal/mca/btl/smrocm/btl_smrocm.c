...
mca_btl_smrocm_register_mem(struct mca_btl_base_module_t *btl,
                            struct mca_btl_base_endpoint_t *endpoint, void *base, size_t size,
                            uint32_t flags);

static int mca_btl_smrocm_deregister_mem(struct mca_btl_base_module_t *btl,
                                         struct mca_btl_base_registration_handle_t *handle);

mca_btl_smrocm_t mca_btl_smrocm = {.super = {
                                       .btl_component = &mca_btl_smrocm_component.super,
                                       .btl_add_procs = mca_btl_smrocm_add_procs,
                                       .btl_del_procs = mca_btl_smrocm_del_procs,
                                       .btl_finalize = mca_btl_smrocm_finalize,
                                       .btl_alloc = mca_btl_smrocm_alloc,
                                       .btl_free = mca_btl_smrocm_free,
                                       .btl_prepare_src = mca_btl_smrocm_prepare_src,
                                       .btl_register_mem = mca_btl_smrocm_register_mem,
                                       .btl_deregister_mem = mca_btl_smrocm_deregister_mem,
                                       .btl_send = mca_btl_smrocm_send,
                                       .btl_sendi = mca_btl_smrocm_sendi,
                                       .btl_dump = mca_btl_smrocm_dump,
                                       .btl_register_error = mca_btl_smrocm_register_error_cb,
                                   }};

static void mca_btl_smrocm_send_rocm_ipc_request(struct mca_btl_base_module_t *btl,
...
static void *mpool_calloc(size_t nmemb, size_t size)
{
    ...
    mca_mpool_base_module_t *mpool = mca_btl_smrocm_component.sm_mpool;
...
static int setup_mpool_base_resources(mca_btl_smrocm_component_t *comp_ptr,
...
static int sm_segment_attach(mca_btl_smrocm_component_t *comp_ptr)
{
    int rc = OPAL_SUCCESS;
    int fd = -1;
    ssize_t bread = 0;
...
static int smrocm_btl_first_time_init(mca_btl_smrocm_t *smrocm_btl, int32_t my_smp_rank, int n)
{
    size_t length, length_payload;
    sm_fifo_t *my_fifos;
    int my_mem_node, num_mem_nodes, i, rc;
    mca_common_sm_mpool_resources_t *res = NULL;
    mca_btl_smrocm_component_t *m = &mca_btl_smrocm_component;
    char *loc, *mynuma;
    opal_process_name_t wildcard_rank;

    /* Assume we don't have hwloc support and fill in dummy info */
    mca_btl_smrocm_component.mem_node = my_mem_node = 0;
    mca_btl_smrocm_component.num_mem_nodes = num_mem_nodes = 1;
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        mca_btl_smrocm_component.num_mem_nodes = num_mem_nodes = strtoul(loc, NULL, 10);
        free(loc);
    } else {
        ...
        if (OPAL_SUCCESS == opal_hwloc_base_get_topology()) {
            ...
            /* JMS This tells me how many numa nodes are *available*,
               ...
               used *in this job*. */
            mca_btl_smrocm_component.num_mem_nodes = num_mem_nodes = i;
        }
    }
    ...
    if (OPAL_SUCCESS == rc) {
        if (NULL == loc) {
            mca_btl_smrocm_component.mem_node = my_mem_node = -1;
        } else {
            ...
            if (NULL == mynuma || NULL != strchr(mynuma, ',') || NULL != strchr(mynuma, '-')) {
                ...
                mca_btl_smrocm_component.mem_node = my_mem_node;
            }
            if (NULL != mynuma) {
                free(mynuma);
            }
        ...
        if (OPAL_SUCCESS == opal_hwloc_base_get_topology() && num_mem_nodes > 0
             ...
             */
            if (1 == n_bound) {
                mca_btl_smrocm_component.mem_node = my_mem_node = numa;
            } else {
                mca_btl_smrocm_component.mem_node = my_mem_node = -1;
            }
        }
    ...
    mca_btl_smrocm_component.sm_mpools = (mca_mpool_base_module_t **)
    ...
    res->mem_node = -1;
    res->allocator = mca_btl_smrocm_component.allocator;
    ...
    mca_btl_smrocm_component.sm_mpools[0] = opal_btl_smcuda_common_sm_mpool_create(res);
    /* Sanity check to ensure that we found it */
    if (NULL == mca_btl_smrocm_component.sm_mpools[0]) {
        free(res);
        return OPAL_ERR_OUT_OF_RESOURCE;
    }

    mca_btl_smrocm_component.sm_mpool = mca_btl_smrocm_component.sm_mpools[0];

    mca_btl_smrocm_component.sm_mpool_base = mca_btl_smrocm_component.sm_mpools[0]->mpool_base(
        mca_btl_smrocm_component.sm_mpools[0]);

    /* create a list of peers */
    mca_btl_smrocm_component.sm_peers = (struct mca_btl_base_endpoint_t **)
        calloc(n, sizeof(struct mca_btl_base_endpoint_t *));
    if (NULL == mca_btl_smrocm_component.sm_peers) {
        free(res);
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    /* Register the entire shared memory region with the ROCM library which will
    ...
    opal_output_verbose(10, opal_btl_base_framework.framework_output,
                        "btl:smrocm: ROCM hipHostRegister address=%p, size=%d",
                        mca_btl_smrocm_component.sm_mpool_base, (int) res->size);
    if (0 == strcmp(opal_accelerator_base_selected_component.base_version.mca_component_name, "rocm")) {
        rc = opal_accelerator.host_register(MCA_ACCELERATOR_NO_DEVICE_ID, mca_btl_smrocm_component.sm_mpool_base, res->size); 
        if (OPAL_UNLIKELY(OPAL_SUCCESS != rc)) {
            /* If registering the memory fails, print a message and continue.
             * This is not a fatal error. */
            opal_output_verbose(10, opal_btl_base_framework.framework_output,
                                "btl:smrocm: ROCM hipHostRegister failed");
        }
    }
     ...
     * specified limits */
    if (mca_btl_smrocm_component.sm_max_procs > 0
        && mca_btl_smrocm_component.num_smp_procs + n > mca_btl_smrocm_component.sm_max_procs) {
        return OPAL_ERROR;
    }

    mca_btl_smrocm_component.shm_fifo = (volatile sm_fifo_t **)
                                            mca_btl_smrocm_component.sm_seg->module_data_addr;
    mca_btl_smrocm_component.shm_bases = (char **) (mca_btl_smrocm_component.shm_fifo + n);
    mca_btl_smrocm_component.shm_mem_nodes = (uint16_t *) (mca_btl_smrocm_component.shm_bases + n);

    /* set the base of the shared memory segment */
    mca_btl_smrocm_component.shm_bases[mca_btl_smrocm_component.my_smp_rank]
        = (char *) mca_btl_smrocm_component.sm_mpool_base;
    mca_btl_smrocm_component.shm_mem_nodes[mca_btl_smrocm_component.my_smp_rank] = (uint16_t)
        my_mem_node;
    ...
    mca_btl_smrocm_component.shm_fifo[mca_btl_smrocm_component.my_smp_rank] = my_fifos;

    /* cache the pointer to the 2d fifo array.  These addresses
     * are valid in the current process space */
    mca_btl_smrocm_component.fifo = (sm_fifo_t **) malloc(sizeof(sm_fifo_t *) * n);

    if (NULL == mca_btl_smrocm_component.fifo)
        return OPAL_ERR_OUT_OF_RESOURCE;

    mca_btl_smrocm_component.fifo[mca_btl_smrocm_component.my_smp_rank] = my_fifos;

    mca_btl_smrocm_component.mem_nodes = (uint16_t *) malloc(sizeof(uint16_t) * n);
    if (NULL == mca_btl_smrocm_component.mem_nodes)
        return OPAL_ERR_OUT_OF_RESOURCE;

    /* initialize fragment descriptor free lists */

    /* allocation will be for the fragment descriptor and payload buffer */
    length = sizeof(mca_btl_smrocm_frag1_t);
    length_payload = sizeof(mca_btl_smrocm_hdr_t) + mca_btl_smrocm_component.eager_limit;
    i = opal_free_list_init(&mca_btl_smrocm_component.sm_frags_eager, length, opal_cache_line_size,
                            OBJ_CLASS(mca_btl_smrocm_frag1_t), length_payload, opal_cache_line_size,
                            ...
                            mca_btl_smrocm_component.sm_mpool, 0, NULL, NULL, NULL);
    if (OPAL_SUCCESS != i)
        return i;

    length = sizeof(mca_btl_smrocm_frag2_t);
    length_payload = sizeof(mca_btl_smrocm_hdr_t) + mca_btl_smrocm_component.max_frag_size;
    i = opal_free_list_init(&mca_btl_smrocm_component.sm_frags_max, length, opal_cache_line_size,
                            OBJ_CLASS(mca_btl_smrocm_frag2_t), length_payload, opal_cache_line_size,
                            mca_btl_smrocm_component.sm_free_list_num,
                            mca_btl_smrocm_component.sm_free_list_max,
                            mca_btl_smrocm_component.sm_free_list_inc,
                            mca_btl_smrocm_component.sm_mpool, 0, NULL, NULL, NULL);
    ...
    i = opal_free_list_init(&mca_btl_smrocm_component.sm_frags_user, sizeof(mca_btl_smrocm_user_t),
                            opal_cache_line_size, OBJ_CLASS(mca_btl_smrocm_user_t),
                            sizeof(mca_btl_smrocm_hdr_t), opal_cache_line_size,
                            mca_btl_smrocm_component.sm_free_list_num,
                            mca_btl_smrocm_component.sm_free_list_max,
                            mca_btl_smrocm_component.sm_free_list_inc,
                            mca_btl_smrocm_component.sm_mpool, 0, NULL, NULL, NULL);
    ...
    mca_btl_smrocm_component.num_outstanding_frags = 0;

    mca_btl_smrocm_component.num_pending_sends = 0;
    i = opal_free_list_init(&mca_btl_smrocm_component.pending_send_fl,
                            sizeof(btl_smrocm_pending_send_item_t), 8,
...
static struct mca_btl_base_endpoint_t *create_sm_endpoint(int local_proc, struct opal_proc_t *proc)
{
    ...
    if (NULL == ep)
        return NULL;
    ep->peer_smp_rank = local_proc + mca_btl_smrocm_component.num_smp_procs;
    ...
    if (ep->fifo_fd < 0) {
        opal_output(0, "mca_btl_smrocm_add_procs: open(%s) failed with errno=%d\n", path, errno);
        free(ep);
        return NULL;
    }
#endif
...
int mca_btl_smrocm_add_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                             ...
                             opal_bitmap_t *reachability)
{
    ...
    smrocm_btl = (mca_btl_smrocm_t *) btl;
    ...
    for (proc = 0; proc < (int32_t) nprocs; proc++) {
        ...
        if (my_proc == procs[proc]) {
            my_smp_rank = mca_btl_smrocm_component.my_smp_rank = n_local_procs++;
            continue;
        }
    ...
    if (!smrocm_btl->btl_inited) {
        return_code = smrocm_btl_first_time_init(smrocm_btl, my_smp_rank,
    ...
    for (proc = 0; proc < (int32_t) nprocs; proc++) {
        if (NULL == peers[proc])
            continue;
        mca_btl_smrocm_component.sm_peers[peers[proc]->peer_smp_rank] = peers[proc];
        peers[proc]->my_smp_rank = my_smp_rank;
    }

    bases = mca_btl_smrocm_component.shm_bases;
    sm_mpool_modp = (mca_common_sm_mpool_module_t *) mca_btl_smrocm_component.sm_mpool;
     ...
     */
    for (j = mca_btl_smrocm_component.num_smp_procs;
         j < mca_btl_smrocm_component.num_smp_procs + FIFO_MAP_NUM(n_local_procs); j++) {

        return_code = sm_fifo_init(mca_btl_smrocm_component.fifo_size,
                                   mca_btl_smrocm_component.sm_mpool,
                                   &mca_btl_smrocm_component.fifo[my_smp_rank][j],
     ...
     */
    (void) opal_atomic_add_fetch_32(&mca_btl_smrocm_component.sm_seg->module_seg->seg_inited, 1);
    while (n_local_procs > mca_btl_smrocm_component.sm_seg->module_seg->seg_inited) {
        opal_progress();
        opal_atomic_rmb();
    }
    ...
    if (0 == my_smp_rank) {
        if (OPAL_SUCCESS != mca_common_sm_module_unlink(mca_btl_smrocm_component.sm_seg)) {
        ...
        if (-1 == unlink(mca_btl_smrocm_component.sm_mpool_rndv_file_name)) {
            opal_output(0, "WARNING: %s unlink failed.\n",
                        mca_btl_smrocm_component.sm_mpool_rndv_file_name);
        }
        if (-1 == unlink(mca_btl_smrocm_component.sm_rndv_file_name)) {
            opal_output(0, "WARNING: %s unlink failed.\n",
                        mca_btl_smrocm_component.sm_rndv_file_name);
        }
    }

    /* free up some space used by the name buffers */
    free(mca_btl_smrocm_component.sm_mpool_ctl_file_name);
    free(mca_btl_smrocm_component.sm_mpool_rndv_file_name);
    free(mca_btl_smrocm_component.sm_ctl_file_name);
    free(mca_btl_smrocm_component.sm_rndv_file_name);

    /* coordinate with other processes */
    for (j = mca_btl_smrocm_component.num_smp_procs;
         j < mca_btl_smrocm_component.num_smp_procs + n_local_procs; j++) {
        ptrdiff_t diff;

        /* spin until this element is allocated */
        /* doesn't really wait for that process... FIFO might be allocated, but not initialized */
        opal_atomic_rmb();
        while (NULL == mca_btl_smrocm_component.shm_fifo[j]) {
            opal_progress();
            opal_atomic_rmb();
        }
        ...
        mca_btl_smrocm_component.fifo[j] = (sm_fifo_t *)
            OFFSET2ADDR(diff, mca_btl_smrocm_component.shm_fifo[j]);

        /* cache local copy of peer memory node number */
        mca_btl_smrocm_component.mem_nodes[j] = mca_btl_smrocm_component.shm_mem_nodes[j];
    }

    /* update the local smp process count */
    mca_btl_smrocm_component.num_smp_procs += n_local_procs;

    /* make sure we have enough eager fragmnents for each process */
    return_code = opal_free_list_resize_mt(&mca_btl_smrocm_component.sm_frags_eager,
                                           mca_btl_smrocm_component.num_smp_procs * 2);
    if (OPAL_SUCCESS != return_code)
        goto CLEANUP;
...
int mca_btl_smrocm_del_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                             struct opal_proc_t **procs, struct mca_btl_base_endpoint_t **peers)
{
...
int mca_btl_smrocm_finalize(struct mca_btl_base_module_t *btl)
{
    return OPAL_SUCCESS;
}
 ...
 */
int mca_btl_smrocm_register_error_cb(struct mca_btl_base_module_t *btl,
                                     mca_btl_base_module_error_cb_fn_t cbfunc)
{
    mca_btl_smrocm_t *smrocm_btl = (mca_btl_smrocm_t *) btl;
    smrocm_btl->error_cb = cbfunc;
    return OPAL_SUCCESS;
}
 ...
 */
extern mca_btl_base_descriptor_t *mca_btl_smrocm_alloc(struct mca_btl_base_module_t *btl,
                                                       struct mca_btl_base_endpoint_t *endpoint,
                                                       uint8_t order, size_t size, uint32_t flags)
{
    mca_btl_smrocm_frag_t *frag = NULL;
    if (size <= mca_btl_smrocm_component.eager_limit) {
        MCA_BTL_SMROCM_FRAG_ALLOC_EAGER(frag);
    } else if (size <= mca_btl_smrocm_component.max_frag_size) {
        MCA_BTL_SMROCM_FRAG_ALLOC_MAX(frag);
    }
 ...
 */
extern int mca_btl_smrocm_free(struct mca_btl_base_module_t *btl, mca_btl_base_descriptor_t *des)
{
    mca_btl_smrocm_frag_t *frag = (mca_btl_smrocm_frag_t *) des;
 ...
 * @param btl (IN)      BTL module
 */
struct mca_btl_base_descriptor_t *mca_btl_smrocm_prepare_src(
    ...
    struct opal_convertor_t *convertor, uint8_t order, size_t reserve, size_t *size, uint32_t flags)
{
    ...
    if (reserve + max_data <= mca_btl_smrocm_component.eager_limit) {
        MCA_BTL_SMROCM_FRAG_ALLOC_EAGER(frag);
    } else {
        MCA_BTL_SMROCM_FRAG_ALLOC_MAX(frag);
    }
...
#if 0
#    define MCA_BTL_SMROCM_TOUCH_DATA_TILL_CACHELINE_BOUNDARY(sm_frag) \
        do {                                                           \
 ...
 */
int mca_btl_smrocm_sendi(struct mca_btl_base_module_t *btl,
                         struct mca_btl_base_endpoint_t *endpoint,
                         ...
                         mca_btl_base_descriptor_t **descriptor)
{
    ...
    /* Initiate setting up ROCM IPC support. */

    if (0 == strcmp(opal_accelerator_base_selected_component.base_version.mca_component_name, "rocm") && (IPC_INIT == endpoint->ipcstate)
        && mca_btl_smrocm_component.use_rocm_ipc) {
        mca_btl_smrocm_send_rocm_ipc_request(btl, endpoint);
    }
    /* We do not want to use this path when we have ROCM IPC support */
    ...
    if (length < mca_btl_smrocm_component.eager_limit) {
        ...
        */
        ...
        MCA_BTL_SMROCM_TOUCH_DATA_TILL_CACHELINE_BOUNDARY(frag);
  ...
  return_resource_busy:
    if (NULL != descriptor) {
        *descriptor = mca_btl_smrocm_alloc(btl, endpoint, order, length, flags);
    }
 ...
 * @param peer (IN)     BTL peer addressing
 */
int mca_btl_smrocm_send(struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                        struct mca_btl_base_descriptor_t *descriptor, mca_btl_base_tag_t tag)
{
    mca_btl_smrocm_frag_t *frag = (mca_btl_smrocm_frag_t *) descriptor;
    int rc;
...
mca_btl_smrocm_register_mem(struct mca_btl_base_module_t *btl,
                            ...
                            uint32_t flags)
{
    mca_btl_smrocm_t *smrocm_module = (mca_btl_smrocm_t *) btl;
    mca_opal_rocm_reg_t *reg;
    int access_flags = flags & MCA_BTL_REG_FLAG_ACCESS_ANY;
    int rcache_flags = 0;

    smrocm_module->rcache->rcache_register(smrocm_module->rcache, base, size, rcache_flags,
...
{
    mca_btl_smrocm_t *smrocm_module = (mca_btl_smrocm_t *) btl;
    mca_opal_rocm_reg_t *reg = (mca_opal_rocm_reg_t
                                             *) ((intptr_t) handle
                                                 - offsetof(mca_opal_rocm_reg_t, data));

    smrocm_module->rcache->rcache_deregister(smrocm_module->rcache, &reg->base);
 ...
 */
static void mca_btl_smrocm_wait_stream_synchronize(mca_opal_rocm_reg_t *rget_reg)
{
#if OPAL_ROCM_SYNC_MEMOPS
    /* No need for any of this with SYNC_MEMOPS feature */
    return;
#else /* OPAL_ROCM_SYNC_MEMOPS */
    hipIpcEventHandle_t evtHandle;
    hipEvent_t event;
    hipError_t result;

    memcpy(&evtHandle, rget_reg->data.evtHandle, sizeof(evtHandle));

    result = hipIpcOpenEventHandle(&event, evtHandle);
    if (OPAL_UNLIKELY(hipSuccess != result)) {
        opal_output_verbose(10, mca_btl_smrocm_component.rocm_ipc_output,
                            "hipIpcOpenEventHandle failed");
    }

    /* BEGIN of Workaround - There is a bug in ROCM 4.1 RC2 and earlier
     * versions.  Need to record an event on the stream, even though
     * it is not used, to make sure we do not short circuit our way
     * out of the hipStreamWaitEvent test.
     */
    result = hipEventRecord(event, 0);
    if (OPAL_UNLIKELY(hipSuccess != result)) {
        opal_output_verbose(10, mca_btl_smrocm_component.rocm_ipc_output,
    ...
    result = hipStreamWaitEvent(0, event, 0);
    if (OPAL_UNLIKELY(hipSuccess != result)) {
        opal_output_verbose(10, mca_btl_smrocm_component.rocm_ipc_output,
    ...
    result = hipEventDestroy(event);
    if (OPAL_UNLIKELY(hipSuccess != result)) {
        opal_output_verbose(10, mca_btl_smrocm_component.rocm_ipc_output,
                            "hipStreamWaitEvent failed");
    }
#endif /* OPAL_ROCM_SYNC_MEMOPS */
}

int mca_btl_smrocm_get_rocm(struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *ep,
...
{
    mca_opal_rocm_reg_t rget_reg;
    mca_opal_rocm_reg_t *reg_ptr = &rget_reg;
    int rc;
    ...
    MCA_BTL_SMROCM_FRAG_ALLOC_USER(frag);
    if (OPAL_UNLIKELY(NULL == frag)) {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
...
#    endif
    ...
    mca_btl_smrocm_wait_stream_synchronize(&rget_reg);

    rc = mca_btl_smrocm_memcpy(local_address, remote_memory_address, size, "mca_btl_smrocm_get",
...
/**
 * Send a ROCM IPC request message to the peer.  This indicates that this rank
 * is interested in establishing ROCM IPC support between this rank and GPU
                                                 ...
                                                 struct mca_btl_base_endpoint_t *endpoint)
{
    ...
    OPAL_THREAD_ADD_FETCH32(&mca_btl_smrocm_component.num_outstanding_frags, +1);
    opal_output_verbose(10, mca_btl_smrocm_component.rocm_ipc_output,
                        "Sending ROCM IPC REQ (try=%d): myrank=%d, mydev=%d, peerrank=%d",
                        endpoint->ipctries, mca_btl_smrocm_component.my_smp_rank, mydevnum,
 ...
 *
 */
void mca_btl_smrocm_dump(struct mca_btl_base_module_t *btl,
                         struct mca_btl_base_endpoint_t *endpoint, int verbose)
{
    ...
    if (NULL != endpoint) {
        OPAL_LIST_FOREACH (frag, &endpoint->pending_sends, mca_btl_smrocm_frag_t) {
