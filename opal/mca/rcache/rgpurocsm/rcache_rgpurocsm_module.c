 ...
 * GPU memory when using ROCM.  Hence, the name is "rgpurocsm" for "remote
 * ROCM" GPU memory.  There is a cache that can be used to store the
 ...
 * 1. rcache_rgpurocsm_leave_pinned=0, cache_size=unlimited
 * 2. rcache_rgpurocsm_leave_pinned=0, cache_size=limited
 * 3. rcache_rgpurocsm_leave_pinned=1, cache_size=unlimited (default)
 * 4. rcache_rgpurocsm_leave_pinned=1, cache_size=limited.
 *
 ...
 * an error back from the ROCM library saying the memory is in use.
 ...
 * The ROCM library returns an error saying it is already mapped.  To
...
#include "opal/mca/rcache/rgpurocsm/rcache_rgpurocsm.h"
#include <errno.h>
#include <string.h>
#ifdef HAVE_MALLOC_H
#    include <malloc.h>
#endif
#include "opal/include/opal/opal_rocm.h"
 ...
 */
static int mca_rcache_rgpurocsm_open_mem_handle(void *base, size_t size, mca_rcache_base_registration_t *newreg)
{
    hipError_t result;
    hipIpcMemHandle_t *memHandle;
    mca_opal_rocm_reg_t *rocm_newreg = (mca_opal_rocm_reg_t *) newreg;

    /* Save in local variable to avoid ugly casting */
    memHandle = (hipIpcMemHandle_t *) rocm_newreg->data.memHandle;

    /* Open the memory handle and store it into the registration structure. */
    result = hipIpcOpenMemHandle((hipDeviceptr_t *) &newreg->alloc_base, *memHandle,
     ...
     * to clear them out. */
    if (hipErrorAlreadyMapped == result) {
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "ROCM: hipIpcOpenMemHandle returned hipErrorAlreadyMapped for "
    ...
    if (OPAL_UNLIKELY(hipSuccess != result)) {
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "ROCM: hipIpcOpenMemHandle failed: base=%p (remote base=%p,size=%d)",
                            newreg->alloc_base, base, (int) size);
        /* Currently, this is a non-recoverable error */
        return OPAL_ERROR;
    } else {
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "ROCM: hipIpcOpenMemHandle passed: base=%p (remote base=%p,size=%d)",
                            newreg->alloc_base, base, (int) size);
    }
 ...
 */
static int mca_rcache_rgpurocsm_close_mem_handle(void *reg_data, mca_rcache_base_registration_t *reg)
{
    hipError_t result;
    mca_opal_rocm_reg_t *rocm_reg = (mca_opal_rocm_reg_t *) reg;

    result = hipIpcCloseMemHandle((hipDeviceptr_t) rocm_reg->base.alloc_base);
    if (OPAL_UNLIKELY(hipSuccess != result)) {
        if (hipErrorDeinitialized != result) {
            opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "ROCM: hipIpcCloseMemHandle failed: base=%p",
                            rocm_reg->base.alloc_base);
        }
        /* We will just continue on and hope things continue to work. */
    } else {
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "ROCM: hipIpcCloseMemHandle passed: base=%p",
                            rocm_reg->base.alloc_base);
    }
...
static int mca_rcache_rgpurocsm_deregister_no_lock(struct mca_rcache_base_module_t *,
                                                mca_rcache_base_registration_t *);
static inline bool mca_rcache_rgpurocsm_deregister_lru(mca_rcache_base_module_t *rcache)
{
    mca_rcache_rgpurocsm_module_t *rcache_rgpurocsm = (mca_rcache_rgpurocsm_module_t *) rcache;
    mca_rcache_base_registration_t *old_reg;
    int rc;

    /* Remove the registration from the cache and list before
       deregistering the memory */
    old_reg = (mca_rcache_base_registration_t *) opal_list_remove_first(&rcache_rgpurocsm->lru_list);
    if (NULL == old_reg) {
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: The LRU list is empty. There is nothing to deregister");
        return false;
    }

    mca_rcache_base_vma_delete(rcache_rgpurocsm->vma_module, old_reg);

    /* Drop the rcache lock while we deregister the memory */
    OPAL_THREAD_UNLOCK(&rcache->lock);
    assert(old_reg->ref_count == 0);
    rc = mca_rcache_rgpurocsm_close_mem_handle(NULL, old_reg);
    ...
    if (OPAL_SUCCESS != rc) {
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: Failed to deregister the memory addr=%p, size=%d",
                            old_reg->base, (int) (old_reg->bound - old_reg->base + 1));
        return false;
    }

    opal_free_list_return(&rcache_rgpurocsm->reg_list, (opal_free_list_item_t *) old_reg);
    rcache_rgpurocsm->stat_evicted++;
 ...
 */
void mca_rcache_rgpurocsm_module_init(mca_rcache_rgpurocsm_module_t *rcache)
{
    rcache->super.rcache_component = &mca_rcache_rgpurocsm_component.super;
    rcache->super.rcache_register = mca_rcache_rgpurocsm_register;
    rcache->super.rcache_find = mca_rcache_rgpurocsm_find;
    rcache->super.rcache_deregister = mca_rcache_rgpurocsm_deregister;
    rcache->super.rcache_finalize = mca_rcache_rgpurocsm_finalize;
 ...
 */
int mca_rcache_rgpurocsm_register(mca_rcache_base_module_t *rcache, void *addr, size_t size,
                               uint32_t flags, int32_t access_flags,
                               mca_rcache_base_registration_t **reg)
{
    mca_rcache_rgpurocsm_module_t *rcache_rgpurocsm = (mca_rcache_rgpurocsm_module_t *) rcache;
    mca_opal_rocm_reg_t *rgpurocsm_reg;
    mca_opal_rocm_reg_t *rget_reg;
    opal_free_list_item_t *item;
    int rc;
    int mypeer; /* just for debugging */

    /* In order to preserve the signature of the mca_rcache_rgpurocsm_register
    ...
    rget_reg = (mca_opal_rocm_reg_t *) *reg;
    ...
    if (!mca_rcache_rgpurocsm_component.leave_pinned
        && 0 == mca_rcache_rgpurocsm_component.rcache_size_limit) {
        item = opal_free_list_get(&rcache_rgpurocsm->reg_list);
        if (NULL == item) {
            return OPAL_ERR_OUT_OF_RESOURCE;
        }
        rgpurocsm_reg = (mca_opal_rocm_reg_t *) item;
        rgpurocsm_reg->base.rcache = rcache;
        rgpurocsm_reg->base.base = addr;
        rgpurocsm_reg->base.bound = (unsigned char *) addr + size - 1;
        ;
        rgpurocsm_reg->base.flags = flags;

        /* Copy the memory handle received into the registration */
        memcpy(rgpurocsm_reg->data.memHandle, rget_reg->data.memHandle,
        ...
        rc = mca_rcache_rgpurocsm_open_mem_handle(addr, size, (mca_rcache_base_registration_t *) rgpurocsm_reg);

        /* This error should not happen with no cache in use. */
        assert(OPAL_ERR_WOULD_BLOCK != rc);

        if (rc != OPAL_SUCCESS) {
            opal_free_list_return(&rcache_rgpurocsm->reg_list, item);
            return rc;
        }
        rgpurocsm_reg->base.ref_count++;
        *reg = (mca_rcache_base_registration_t *) rgpurocsm_reg;
        return OPAL_SUCCESS;
    }

    /* Check to see if memory is registered and stored in the cache. */
    OPAL_THREAD_LOCK(&rcache->lock);
    mca_rcache_base_vma_find(rcache_rgpurocsm->vma_module, addr, size, reg);
    ...
    if (*reg != NULL) {
        rcache_rgpurocsm->stat_cache_hit++;
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: Found addr=%p,size=%d (base=%p,size=%d) in cache", addr,
                            (int) size, (*reg)->base, (int) ((*reg)->bound - (*reg)->base));

        if (0 ==
            memcmp(((mca_opal_rocm_reg_t *)*reg)->data.memHandle, rget_reg->data.memHandle,
                  sizeof(((mca_opal_rocm_reg_t *)*reg)->data.memHandle))) {
            /* Registration matches what was requested.  All is good. */
            rcache_rgpurocsm->stat_cache_valid++;
        } else {
            /* This is an old registration.  Need to boot it. */
            opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                                "RGPUROCSM: Mismatched Handle: Evicting/unregistering "
            ...
            if (mca_rcache_rgpurocsm_component.leave_pinned) {
                opal_list_remove_item(&rcache_rgpurocsm->lru_list, (opal_list_item_t *) (*reg));
            }
            ...
            mca_rcache_rgpurocsm_deregister_no_lock(rcache, *reg);
            *reg = NULL;
            rcache_rgpurocsm->stat_cache_invalid++;
        }
    } else {
        /* Nothing was found in the cache. */
        rcache_rgpurocsm->stat_cache_miss++;
    }

    /* If we have a registration here, then we know it is valid. */
    if (*reg != NULL) {
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: CACHE HIT is good: ep=%d, addr=%p, size=%d in cache", mypeer,
                            addr, (int) size);

        /* When using leave pinned, we keep an LRU list. */
        if ((0 == (*reg)->ref_count) && mca_rcache_rgpurocsm_component.leave_pinned) {
            opal_output_verbose(20, mca_rcache_rgpurocsm_component.output,
                                "RGPUROCSM: POP OFF LRU: ep=%d, addr=%p, size=%d in cache", mypeer,
                                addr, (int) size);
            opal_list_remove_item(&rcache_rgpurocsm->lru_list, (opal_list_item_t *) (*reg));
        }
        (*reg)->ref_count++;
        ...
        opal_output_verbose(80, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: Found entry in cache addr=%p, size=%d", addr, (int) size);
        return OPAL_SUCCESS;
    }
    ...
    assert(NULL == *reg);
    opal_output_verbose(
        10, mca_rcache_rgpurocsm_component.output,
        "RGPUROCSM: New registration ep=%d, addr=%p, size=%d. Need to register and insert in cache",
    ...
    memcpy(rgpurocsm_reg->data.memHandle, rget_reg->data.memHandle, sizeof(rget_reg->data.memHandle));
     ...
     * ROCM codes attempt to register the memory.  The case that this
     ...
     * registered and call into ROCM to register it.  However, that
     * will fail with hipErrorAlreadyMapped.  Therefore we need to
    ...
    if (OPAL_ERR_WOULD_BLOCK == rc) {
        ...
        mca_rcache_base_vma_find(rcache_rgpurocsm->vma_module, addr, 4, &oldreg);
        ...
        if (NULL != oldreg) {
            ...
            if (mca_rcache_rgpurocsm_component.leave_pinned) {
                opal_list_remove_item(&rcache_rgpurocsm->lru_list, (opal_list_item_t *) oldreg);
            }
            ...
            mca_rcache_rgpurocsm_deregister_no_lock(rcache, oldreg);
            rcache_rgpurocsm->stat_evicted++;
        ...
        while (OPAL_SUCCESS != rc) {
            if (true != mca_rcache_rgpurocsm_deregister_lru(rcache)) {
                rc = OPAL_ERROR;
                break;
            }
    ...
    opal_output_verbose(80, mca_rcache_rgpurocsm_component.output,
                        "RGPUROCSM: About to insert in rgpurocsm cache addr=%p, size=%d", addr,
                        (int) size);
    rc = mca_rcache_base_vma_insert(rcache_rgpurocsm->vma_module,
                                    (mca_rcache_base_registration_t *) rgpurocsm_reg,
                                    mca_rcache_rgpurocsm_component.rcache_size_limit);
    if (OPAL_ERR_TEMP_OUT_OF_RESOURCE == rc) {
        opal_output_verbose(40, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: No room in the cache - boot the first one out");
        (void) mca_rcache_rgpurocsm_deregister_lru(rcache);
        if (mca_rcache_rgpurocsm_component.empty_cache) {
            int remNum = 1;
            /* Empty out every registration from LRU until it is empty */
            opal_output_verbose(40, mca_rcache_rgpurocsm_component.output,
                                "RGPUROCSM: About to delete all the unused entries in the cache");
            while (mca_rcache_rgpurocsm_deregister_lru(rcache)) {
                remNum++;
            }
            opal_output_verbose(40, mca_rcache_rgpurocsm_component.output,
                                "RGPUROCSM: Deleted and deregistered %d entries", remNum);
            rc = mca_rcache_base_vma_insert(rcache_rgpurocsm->vma_module,
                                            (mca_rcache_base_registration_t *) rgpurocsm_reg,
                                            mca_rcache_rgpurocsm_component.rcache_size_limit);
        } else {
            /* Check for room after one removal. If not, remove another one until there is space */
            while ((rc = mca_rcache_base_vma_insert(rcache_rgpurocsm->vma_module,
                                                    (mca_rcache_base_registration_t *) rgpurocsm_reg,
                                                    mca_rcache_rgpurocsm_component.rcache_size_limit))
                   == OPAL_ERR_TEMP_OUT_OF_RESOURCE) {
                opal_output_verbose(40, mca_rcache_rgpurocsm_component.output,
                                    "RGPUROCSM: No room in the cache - boot one out");
                if (!mca_rcache_rgpurocsm_deregister_lru(rcache)) {
                    break;
                }
            }
        }
    ...
    if (rc != OPAL_SUCCESS) {
         ...
         */
        opal_output_verbose(10, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: Failed to register addr=%p, size=%d", addr, (int) size);
        return OPAL_ERROR;
    }
...
int mca_rcache_rgpurocsm_find(struct mca_rcache_base_module_t *rcache, void *addr, size_t size,
                           mca_rcache_base_registration_t **reg)
{
    mca_rcache_rgpurocsm_module_t *rcache_rgpurocsm = (mca_rcache_rgpurocsm_module_t *) rcache;
    int rc;
    ...
    rc = mca_rcache_base_vma_find(rcache_rgpurocsm->vma_module, addr, size, reg);
    if (*reg != NULL && mca_rcache_rgpurocsm_component.leave_pinned) {
        if (0 == (*reg)->ref_count && mca_rcache_rgpurocsm_component.leave_pinned) {
            opal_list_remove_item(&rcache_rgpurocsm->lru_list, (opal_list_item_t *) (*reg));
        }
        rcache_rgpurocsm->stat_cache_found++;
        (*reg)->ref_count++;
    } else {
        rcache_rgpurocsm->stat_cache_notfound++;
    }
    OPAL_THREAD_UNLOCK(&rcache->lock);
...
int mca_rcache_rgpurocsm_deregister(struct mca_rcache_base_module_t *rcache,
                                 mca_rcache_base_registration_t *reg)
{
    mca_rcache_rgpurocsm_module_t *rcache_rgpurocsm = (mca_rcache_rgpurocsm_module_t *) rcache;
    int rc = OPAL_SUCCESS;
    ...
    if (mca_rcache_rgpurocsm_component.leave_pinned && registration_is_cachebale(reg)) {
        /* if leave_pinned is set don't deregister memory, but put it
         * on LRU list for future use */
        opal_output_verbose(20, mca_rcache_rgpurocsm_component.output,
                            "RGPUROCSM: Deregister: addr=%p, size=%d: cacheable and pinned, leave in "
                            "cache, PUSH IN LRU",
                            reg->base, (int) (reg->bound - reg->base + 1));
        opal_list_prepend(&rcache_rgpurocsm->lru_list, (opal_list_item_t *) reg);
    } else {
        /* Remove from rcache first */
        if (!(reg->flags & MCA_RCACHE_FLAGS_CACHE_BYPASS))
            mca_rcache_base_vma_delete(rcache_rgpurocsm->vma_module, reg);
        ...
        {
            assert(reg->ref_count == 0);
            rc = mca_rcache_rgpurocsm_close_mem_handle(NULL, reg);
        }

        OPAL_THREAD_LOCK(&rcache->lock);

        if (OPAL_SUCCESS == rc) {
            opal_free_list_return(&rcache_rgpurocsm->reg_list, (opal_free_list_item_t *) reg);
        }
    }
    OPAL_THREAD_UNLOCK(&rcache->lock);
...
int mca_rcache_rgpurocsm_deregister_no_lock(struct mca_rcache_base_module_t *rcache,
                                         mca_rcache_base_registration_t *reg)
{
    mca_rcache_rgpurocsm_module_t *rcache_rgpurocsm = (mca_rcache_rgpurocsm_module_t *) rcache;
    int rc = OPAL_SUCCESS;
...
#define RGPUROCSM_RCACHE_NREGS 100

void mca_rcache_rgpurocsm_finalize(struct mca_rcache_base_module_t *rcache)
{
    mca_rcache_rgpurocsm_module_t *rcache_rgpurocsm = (mca_rcache_rgpurocsm_module_t *) rcache;
    mca_rcache_base_registration_t *reg;
    mca_rcache_base_registration_t *regs[RGPUROCSM_RCACHE_NREGS];
    int reg_cnt, i;
    int rc;

    /* Statistic */
    if (true == mca_rcache_rgpurocsm_component.print_stats) {
        opal_output(0,
                    "%s rgpurocsm: stats "
                    "(hit/valid/invalid/miss/evicted): %d/%d/%d/%d/%d\n",
                    OPAL_NAME_PRINT(OPAL_PROC_MY_NAME), rcache_rgpurocsm->stat_cache_hit,
                    rcache_rgpurocsm->stat_cache_valid, rcache_rgpurocsm->stat_cache_invalid,
                    rcache_rgpurocsm->stat_cache_miss, rcache_rgpurocsm->stat_evicted);
    }

    OPAL_THREAD_LOCK(&rcache->lock);
    do {
        reg_cnt = mca_rcache_base_vma_find_all(rcache_rgpurocsm->vma_module, 0, (size_t) -1, regs,
                                               RGPUROCSM_RCACHE_NREGS);
        ...
        for (i = 0; i < reg_cnt; i++) {
            ...
            } else if (mca_rcache_rgpurocsm_component.leave_pinned) {
                opal_list_remove_item(&rcache_rgpurocsm->lru_list, (opal_list_item_t *) reg);
            }
    ...
    } while (reg_cnt == RGPUROCSM_RCACHE_NREGS);

    OBJ_DESTRUCT(&rcache_rgpurocsm->lru_list);
    OBJ_DESTRUCT(&rcache_rgpurocsm->reg_list);
    OPAL_THREAD_UNLOCK(&rcache->lock);
}
